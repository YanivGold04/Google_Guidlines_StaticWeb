...
// Restore scroll position on page load (only for index.html)
window.addEventListener('load', function() {
    ...
    // Only restore scroll on the home page
    if (shouldRestore === 'true' && window.location.pathname.endsWith('index.html')) {
        setTimeout(() => {
            window.scrollTo({ top: savedScroll, behavior: 'smooth' });
        }, 300);
        sessionStorage.removeItem('shouldRestoreScroll');
    } else {
        // Clear the flag if we're not on the home page
        sessionStorage.removeItem('shouldRestoreScroll');
    }
});